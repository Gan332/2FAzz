    ...
    ui.vertical(|ui| {
        ...
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ui.label(
                        egui::RichText::new("Theme").size(15.0f32).color(app.theme.text_primary()),
                    );
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        let is_dark = app.theme.mode == ThemeMode::Dark;
                        let label = if is_dark { "Dark" } else { "Light" };
                        let btn = egui::Button::new(
                            egui::RichText::new(label).size(14.0f32).color(egui::Color32::WHITE),
                        )
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal_wrapped(|ui| {
                    for (color, name) in presets {
                        ...
                        let btn = egui::Button::new(
                            egui::RichText::new(name).size(12.0f32).color(if is_selected {
                                egui::Color32::WHITE
                            } else {
        ...
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                if ui.button(
                    egui::RichText::new("Import Tokens").size(15.0f32).color(app.theme.accent_color()),
                ).clicked() {
                ...
                ui.painter().line_segment(
                    [
                        egui::pos2(sep_rect.min.x, sep_rect.min.y + 2.0f32),
                        egui::pos2(sep_rect.max.x, sep_rect.min.y + 2.0f32),
                    ],
                    egui::Stroke::new(1.0f32, app.theme.separator()),
                );
                ui.add_space(4.0f32);
