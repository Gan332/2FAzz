...
pub fn show(ui: &mut egui::Ui, app: &mut AuthenticatorApp) {
    ui.vertical(|ui| {
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            ...
                            if ui.button(
                                egui::RichText::new("X").size(12.0f32).color(app.theme.destructive()),
                            ).clicked() {
                                to_delete = Some(i);
                            }
                            ...
                            if !matches!(token.token_type, totp::TokenType::Hotp { .. }) {
                                ...
                                let center = egui::pos2(
                                    ui.cursor().min.x + size / 2.0f32,
                                    ui.cursor().min.y + size / 2.0f32,
                                );
                                let radius = size / 2.0f32;
                                ui.painter().circle_stroke(
                                    center,
                                    radius,
                                    egui::Stroke::new(2.0f32, app.theme.text_secondary()),
                                );
                                    ...
                                    points: (0..=32)
                                        .map(|step| {
                                            let angle = -std::f32::consts::FRAC_PI_2
                                                + filled_angle * step as f32 / 32.0f32;
                                            egui::pos2(
        ...
        ui.add_space(70.0f32);

        ui.with_layout(egui::Layout::right_to_left(egui::Align::Max), |ui| {
            let fab = egui::Button::new(
                egui::RichText::new("+").size(28.0f32).color(egui::Color32::WHITE),
            )
            .fill(app.theme.accent_color())
            .rounding(egui::Rounding::same(28.0f32))
            .min_size(egui::vec2(56.0f32, 56.0f32));
